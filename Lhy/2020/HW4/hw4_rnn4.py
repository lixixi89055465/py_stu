...
if __name__ == "__main__":
    ...
    train_x, y = load_training_data('../images/training_label.txt')
    train_x_no_label = load_training_data('../images/training_nolabel.txt')

    print("loading testing images ...")
    test_x = load_testing_data('../images/testing_data.txt')
        ...
        if epoch % 20 == 19:
            ...
            with torch.no_grad():
                ...
                for i, inputs in enumerate(unlabel):
                    ...
                    zOut = (i * batch_size + (outputs > threshold).nonzero()).view(-1).cpu().tolist()
                    canIndex_input += zOut
                    canLabel_input += [1] * len(zOut)
                    zOut = (i * batch_size + (outputs < (1 - threshold)).nonzero()).view(-1).cpu().tolist()
                    canIndex_input += zOut
                ...
                left = 0
                canIndex_input.sort()
                for i in range(len(unlabel.sampler.data_source.data)):
                    if left >= len(canIndex_input):
                        break
                    if i < canIndex_input[left]:
                        srcIndex.append(i)
                    elif i == canIndex_input[left]:
                        left += 1
                if len(canIndex_input) > 0:
                    srcIndex += [i for i in range(canIndex_input[-1] + 1, len(unlabel.sampler.data_source.data))]
                    unlabel.sampler.data_source.data = unlabel.sampler.data_source.data[srcIndex]

                # targetNum=[i for i in range(len(unlabel.sampler.data_source.images ))].remove() for i in canIndex_input
                # unlabel.sampler.data_source.images = unlabel.sampler.data_source.images[
                #     [i for i in range(len(unlabel.sampler.data_source.images)) if i not in canIndex_input]]
                print('canLabel_input:', len(canLabel_input))
                print('canIndex_input:', len(canIndex_input))
                print('unlabel.sampler.data_source.images.len:', len(unlabel.sampler.data_source.data))
                print('train.sampler.data_source.images:', len(train.sampler.data_source.data))
