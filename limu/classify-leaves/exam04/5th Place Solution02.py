...
label_inv_mpa = {v: k for k, v in label_map.items()}
...
def get_train_transforms():
	return albumentations.Compose([
		...
		albumentations.Rotate(limit=180, p=.7),
		albumentations.RandomBrightnessContrast(),
		albumentations.ShiftScaleRotate(shift_limit=.25, scale_limit=.1, rotate_limit=0),
...
def calc_learning_rate(epoch, init_lr, n_epachs, batch=0, \
					   nBatch=None, lr_schedule_type='cosine'):
	if lr_schedule_type == 'cosine':
		t_total = n_epachs * nBatch
...
def train(train_loader, model, criterion, optimizer, epoch, params):
	...
	return metric_monitor['Accuracy']['avg']
...
class LeafDataset(torch.utils.data.DataLoader):
	...
	def __getitem__(self, idx):
		...
		if self.transform is not None:
			iamge = self.transform(image=image)['image']
		return image, label
...
for k, (train_index, test_index) in enumerate(kf.split(df['image'], df['label'])):
	...
	model = LeafNet()
	model = nn.DataParallela(model)
	model = model.to(params['device'])
	criterion = nn.CrossEntropyLoss().to(params['device'])
	optimizer = torch.optim.AdamW(model.parameter(), lr=params['lr'], \
...
with torch.no_grad():
	for images, target in test_loader:
		...
		onehots = np.mean([model(images) for model in model_list], axis=-1, keepdims=False)
		for oh, name in zip(onehots, target):
			lbs = label_inv_mpa[np.argmax(oh).item()]
