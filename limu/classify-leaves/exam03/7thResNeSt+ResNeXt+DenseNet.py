...
def train1(k_folds, num_epochs=30):
...
df_resnest = pd.read_csv('./classify-leaves-results/submission-resnest.csv')
df_resnext = pd.read_csv('./classify-leaves-results/submission-resnext.csv')
df_densenet = pd.read_csv('./classify-leaves-results/submission-densenet.csv')
...
for rows in range(len(df_all)):
	if (df_all['label_resnest'].iloc[rows] == df_all['label_resnext'].iloc[rows]) or \
			(df_all['label_resnext'].iloc[rows] == df_all['label_densenet'].iloc[rows]):
