...
def train(model, dataloaders, epochs, criterion, optimizer):
    ...
    plt.plot(list(range(len(train_accs))), train_accs, label='train accuracy')
    plt.plot(list(range(len(train_accs))), test_accs, label='test accuracy')
    plt.xlabel(' epochs')
    plt.ylabel(' acc ')
    plt.savefig('acc.png')
    plt.show()
...
dataloaders, class_names, predict_dataset = load_dataset(batch_size=128, img_size=256)
net, preds = model, []
test_loader = DataLoader(predict_dataset, batch_size=batch_size, shuffle=False, \
