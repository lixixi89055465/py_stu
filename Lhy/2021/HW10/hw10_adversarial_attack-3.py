...
adv_set = AdvDataset(data_dir=root, transform=transform)
adv_names = adv_set.__getname__()
adv_loader = DataLoader(adv_set, batch_size=batch_size, shuffle=False,num_workers=2)
...
def ifgsm(model, x, y, loss_fn, epsilon=epsilon * 0.01, alpha=alpha, num_iter=50):
    x_adv = x
    for i in range(num_iter):
        ...
        # fgsm: use gradient ascent on x_adv to maximize loss
        x_adv = x_adv + epsilon * x_adv.grad.detach().sign()
        x_adv = torch.max(x - alpha, x_adv)
        x_adv = torch.min(x + alpha, x_adv)
