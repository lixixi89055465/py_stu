...
# error = loss(torch.ones(3, 4, 10), torch.ones((3, 4), dtype=torch.long),
# 			 torch.tensor([4, 2, 0]))
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
	...
	for epoch in range(num_epochs):
		timer = d2l.Timer()
		metric = d2l.Accumulator(2)  # 训练损失总和，词源数量
		for batch in data_iter:
			...
			l.sum().backward()  # s损失函数的标量进行“反向传播"
...
def predict_seq2seq(net, src_sentence, src_vocab, tgt_vocab, \
					num_steps, device, save_attention_weight=False):
	net.eval()
	...
	dec_state = net.encoder.init_state(enc_outputs, enc_valid_len)
	# 添加批量轴
	...
	return ' '.join(tgt_vocab.to_tokens(output)), attention_weight_seq
