...
def get_params(vocab_size, num_hiddens, device):
	...
	W_xh = normal22((num_inputs, num_hiddens))
	W_hh = normal22((num_inputs, num_hiddens))
	b_h = torch.zeros((num_hiddens), device=device)
	W_hq = normal22((num_hiddens, num_outputs))
...
def rnn(inputs, state, params):
	...
	outputs = []
	for X in inputs:
		H = torch.tanh(torch.mm(X, W_xh) + torch.mm(X, W_hh) + b_h)
...
def train_ch8(net, train_iter, vocab, lr, num_epochs, device, \
	...
	print(f'困惑都{ppl:.1f}, {speed:.1f} 词元/秒 {str(device)}')
	print(predict('time traveller'))
