...
def evaluate(data, output):
    ...
    for k in range(num_of_windows):
        ...
        start_prob, start_index = output.start_logits[k].topk(output.start_logits.size(1), dim=0)
        end_prob, end_index = output.end_logits[k].topk(output.end_logits.size(1), dim=0)
        ...
        while i < len(prob):
            if start_index[i] <= end_index[i]:
                start_index, end_index = start_index[i], end_index[i]
                prob = prob[i]
                break
            i += 1
...
result_file = "result_hw7_bert_09.csv"
